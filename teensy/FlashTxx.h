...
#include <stdint.h>     // uint32_t, etc.
  ...
  #define FLASH_SECTOR_SIZE	(0x1000)		// 4KB sector size
  #define FLASH_WRITE_SIZE	(4)			// 4-byte/32-bit writes
  // LOCAL MODIFICATION (upstream is 4*FLASH_SECTOR_SIZE): the Teensy 4.1
  // EEPROM emulation occupies the top 64 sectors of flash (256KB, base
  // 0x607C0000; see cores/teensy4/eeprom.c). Aiie persists prefs there, so
  // reserving only 16KB made firmware_buffer_init()'s "scan down to top of
  // code" stop inside the EEPROM data and report a tiny buffer ("image too
  // large"). Reserve the whole EEPROM region so the buffer ends below it.
